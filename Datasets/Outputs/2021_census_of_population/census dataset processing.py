...
# Input directory
base_path = '../../Inputs/2021_census_of_population/'  # Adjust to your directory
...
gender = pd.read_csv('../../Inputs/2021_census_of_population/98100032.csv') # 98100032.csv
...
# Print final stats
print(f"Number of rows in the municipalities dataset: {len(municipalities_merged)} "
      f"(number of columns: {len(municipalities_merged.columns)})")

# Clean CSDUID column
municipalities_merged["CSDUID"] = municipalities_merged["CSDUID"].astype(str).str.replace(r"^2021A0005", "", regex=True)

# Save to a CSV
municipalities_merged.to_csv('2021_census_of_population_municipalities.csv', index=False)
print("Saved to '2021_census_of_population_municipalities.csv'\n")

#endregion

## ----------------------------------------- Statistics of census subdivisions -----------------------------------------
#region

for indigenous in indigenous_types:
    if indigenous in merged.columns and 'Indigenous identity: total count' in merged.columns:
        merged[f'{indigenous} (%)'] = (merged[indigenous] / merged['Indigenous identity: total count'] * 100).round(2)

for dwelling in dwelling_types:
    if dwelling in merged.columns and 'Total Dwellings by Type' in merged.columns:
        merged[f'{dwelling} (%)'] = (merged[dwelling] / merged['Total Dwellings by Type'] * 100).round(2)

for minority in minority_types:
    if minority in merged.columns and 'Visible minority: total counts' in merged.columns:
        merged[f'{minority} (%)'] = (merged[minority] / merged['Visible minority: total counts'] * 100).round(2)

# Create percentage for men
if 'Number of Men' in merged.columns and 'Population, 2021' in merged.columns:
    merged['Number of Men (%)'] = (merged['Number of Men'] / merged['Population, 2021'] * 100).round(2)

if 'Men in Workforce' in merged.columns and 'People in Workforce' in merged.columns:
    merged['Men in Workforce (%)'] = (merged['Men in Workforce'] / merged['People in Workforce'] * 100).round(2)

# Print final stats
print(f"Number of rows in the CSD dataset: {len(merged)} (number of columns: {len(merged.columns)})")

# Save to a CSV
merged.to_csv('2021_census_of_population_csds.csv', index=False)
print("Saved to '2021_census_of_population_csds.csv'")
